 ...
 */
int TraceInit();
int TraceInitNodeName(const char* node_name); //禁用，不再使用！！！！！！！
void TraceDestroy();
...
void TraceSetModuleId(uint16_t module_id);
void TraceSetModuleType(uint8_t type);
//result: 1: 失败； 0：成功
void TraceChek(int result, int type, uint16_t module_id, uint16_t sub_id, const char* format, ...);
#ifdef __cplusplus
}
#endif
