...
int removeFileList(char *basePath, int reserved_day=2)
{
    DIR *dir;
    struct dirent *ptr;
    char path[1000];
    ...
    {
        perror("removeFileList Open dir error...");
        exit(1);
    }
                           ...
                           int divide_log=0)
{
    ...
    if (f != NULL)
    {
        ...
        {
            strcpy(logname, "/data/log/");
            strcat(logname, f);
            strcat(logname, "/");
        }
        // 创建新日志文件夹
        if(access(logname, 0)==-1)
            mkdir(logname, 0744);
                    // 删除超限日志

        removeFileList(logname, reserved_day);
        // 新日志名称
        strcat(logname, f);
        strcat(logname, "_");
        char t[32] = {0};
        ...
        if(m_divide_log == 1)
        {
            char info_logname[256] = {0};
            strcpy(info_logname, logname);
            strcat(info_logname, ".log.info");
            m_log_info_fp = fopen(info_logname, "a");
            fprintf(m_log_info_fp, "\n");
            char error_logname[256] = {0};
            strcpy(error_logname, logname);
            strcat(error_logname, ".log.error");
            m_log_error_fp = fopen(error_logname, "a");
            fprintf(m_log_error_fp, "\n");
            char debug_logname[256] = {0};
            strcpy(debug_logname, logname);
            strcat(debug_logname, ".log.debug");
            m_log_debug_fp = fopen(debug_logname, "a");
            fprintf(m_log_debug_fp, "\n");
        }
        else
        {
            strcat(logname, ".log");
...
FILE* TraceLog::getTraceLog(int log_mode=0)
{
    if(log_mode == 1)
...
char *GetCurrentTime(char *t) {
  ...
  sprintf(t, "%4d-%02d-%02d %02d:%02d:%02d.%06ld", 1900 + _now->tm_year,
          1 + _now->tm_mon, _now->tm_mday, _now->tm_hour, _now->tm_min,
          _now->tm_sec, _tv.tv_usec);
  return t;
}
