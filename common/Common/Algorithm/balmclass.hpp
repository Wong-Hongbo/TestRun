...
inline void down_sampling_voxel(PL_VEC &pl_feat, double voxel_size)
{
  unordered_map<VOXEL_LOC, M_POINT> feat_map;
  uint plsize = pl_feat.size();
...
inline void plvec_trans_func(vector<Eigen::Vector3d> &orig, vector<Eigen::Vector3d> &tran, Eigen::Matrix3d R, Eigen::Vector3d t)
{
...
inline double feat_eigen_limit[2] = {3*3, 2*2};
...
public:
  ...
  vector<MYSO3> so3_poses, so3_poses_temp;
  ...
  void acc_t_evaluate(vector<MYSO3> &so3_ps, vector<Eigen::Vector3d> &t_ps, int head, int end, Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT, double &residual)
  {
    ...
    for(int a=head; a<end; a++)
    {
      ...
      for(uint i=0; i<backnum; i++)
      {
        ...
        point_xis[i] = -MYSO3::hat(vec_tran);
  ...
  void divide_thread(vector<MYSO3> &so3_ps, vector<Eigen::Vector3d> &t_ps,Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT, double &residual)
  {
    Hess.setZero(); JacT.setZero(); residual = 0;
  ...
  // Calculate residual
  void evaluate_only_residual(vector<MYSO3> &so3_ps, vector<Eigen::Vector3d> &t_ps, double &residual)
  {
    residual = 0;
    uint gps_size = plvec_voxels.size();
    Eigen::Vector3d vec_tran;
...
//          Vector3d  test = dxi.block<3, 1>(6*(j), 0);
//          std::"dxi.block<3, 1>(6*(j) : "  << test << std::endl;

        // left multiplication
        so3_poses_temp[j] = MYSO3::exp(dxi.block<3, 1>(6*(j), 0)) * so3_poses[j];
...
public:
  ...
  void evaluate_para(MYSO3 &so3_p, Eigen::Vector3d &t_p, Eigen::Matrix<double, 6, 6> &Hess, Eigen::Matrix<double, 6, 1> &g, double &residual)
  {
    ...
    for(uint i=0; i<a_size; i++)
    {
      ...
      Eigen::Matrix3d point_xi = -MYSO3::hat(vec_tran);
      vec_tran += t_p;
  ...
  void evaluate_only_residual(MYSO3 &so3_p, Eigen::Vector3d &t_p, double &residual)
  {
    residual = 0;
    uint a_size = surf_gather.size();
    for(uint i=0; i<a_size; i++)
  ...
  void damping_iter()
  {
    ...
    for(int i=0; i<20; i++)
    {
      ...
      so3_temp = MYSO3::exp(dxi.block<3, 1>(0, 0)) * so3_pose;
