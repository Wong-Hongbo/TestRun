...
void SCANTOMAP::ResetSystem()
{
    ...
    bReset = false;
    bBA_init = false;
    _current_pose = _previous_gps = _pre_keypose;
    sensor_height = 0;
    tilt_deg = 0;
...
void SCANTOMAP::Matching()
{
    ...
    //    std::cout << "_target_points  size : " << _target_points->size() << " resolution : " << registration->getResolution()  << " stepsize : " << registration->getStepSize() << std::endl;

    t_localizer  =registration->getFinalTransformation();
    //    std::cout << "t_localizer : " << t_localizer << std::endl;
    //    t_localizer = align_matrix *initguess_matrix;
...
bool SCANTOMAP::Process(HDLADARDATA_MSG* lidar_data)
{
    ...
    _current_pose.roll = 0;
    _current_pose.pitch = 0;
...
//            pcl::io::savePCDFileBinaryCompressed(saveRawKeyFrame + "/" + std::to_string(save_count) + ".pcd",*scan_ptr);  //保存当前原始关键幀结果
            save_count++;
        }
    }
...
//void SCANTOMAP::AddOdomFactor(const RPYpose current_pose, const RPYpose previous_pose, const int id)
//{
//    mBackEnd->addOdomConstraint(current_pose,previous_pose,id);
//}
...
//        vector6_prior << 0.5,0.5,0.5,0.1,0.1,0.1;
        ...
        //        noiseModel::Diagonal::shared_ptr priorNoise = noiseModel::Diagonal::Variances((Vector(6) << 1.e-6, 1.e-6,1.e-6, 1.e-6, 1.e-6, 1.e-6).finished()); // rad*rad, meter*meter
...
void SCANTOMAP::CorrectPose()
{
    if(_loop_closed )
    {
        ...
        int num_poses = mMapDataFrame->cloud_keyposes_6d_ptr->size();
        ...
        for (int i = 0; i < num_poses; ++i)
        {
            ...
            //            cloud_keyposes_2d_->points[i].z = 0;
        ...
        std::cout << "_current_pose x : " << _current_pose.x << std::endl;
        std::cout << "_current_pose y : " << _current_pose.y << std::endl;
        std::cout << "_current_pose z : " << _current_pose.z << std::endl;
        std::cout << "_current_pose roll : " << _current_pose.roll << std::endl;
        std::cout << "_current_pose pitch : " << _current_pose.pitch << std::endl;
        std::cout << "_current_pose yaw : " << _current_pose.yaw << std::endl;
...
void SCANTOMAP::GetInsAndLpInfo()
{
    ...
    current_localpose.yaw = (lidardata->LocalPose.dr_heading-9000)*M_PI/18000.0f+2*M_PI;
...
//    icp.setMaxCorrespondenceDistance(search_loop_radius*2);
...
//    Pose3 latest_estimate=isam_current_estimate_.at<Pose3>(gtsam::symbol('x',size);
    ...
    // Add pose constraint
    //    mtx.lock();
    //    loop_Index_Queue.push_back(make_pair(loopKeyCur, loopKeyCur));
    //    loop_Pose_Queue.push_back(poseFrom.between(poseTo));
    //    loop_Noise_Queue.push_back(constraintNoise);
    //    loop_Pose_from_Queue.push_back(ryp_pose_from);
    //    loop_Pose_to_Queue.push_back(ryp_pose_to);
    //    mtx.unlock();
