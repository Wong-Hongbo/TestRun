...
//    IsamOptimizer->ResetIsam();
    ...
    double scale_position = 0.1;
    double scale_rotation = 0.1;
...
void BACKEND::CorrectKeyPose()
{
    ...
    {
        mMapDataFrame->cloud_keyposes_6d_ptr->points[i].x = mMapDataFrame->cloud_keyposes_3d_ptr->points[i].x = mMapDataFrame->cloud_keyposes_2d_ptr->points[i].x = this->isam_current_estimate.at<Pose3>(gtsam::symbol('x',i)).translation().x();
        mMapDataFrame->cloud_keyposes_6d_ptr->points[i].y = mMapDataFrame->cloud_keyposes_3d_ptr->points[i].y = mMapDataFrame->cloud_keyposes_2d_ptr->points[i].y = this->isam_current_estimate.at<Pose3>(gtsam::symbol('x',i)).translation().y();
        mMapDataFrame->cloud_keyposes_6d_ptr->points[i].z = mMapDataFrame->cloud_keyposes_3d_ptr->points[i].z =  this->isam_current_estimate.at<Pose3>(gtsam::symbol('x',i)).translation().z();
    ...
    pcl::io::savePCDFileBinary("correct_keypose.pcd",*mMapDataFrame->cloud_keyposes_6d_ptr);
}
