/**
* Copyright(c)2017 ChangSha XingShen Technology Ltd .
* All rights reserved
* @projectName  CloudCompare-Qt
* @fileName     m2dp_yhl.cc
* @brief
* @author       Yu Hui Liang
* @date         2020-09-17 17:50:21
*/

#include "Common/Algorithm/m2dp_yhl.hh"


M2DP::M2DP(double r, double max_distance, int32_t t, int32_t p,
                      int32_t q):
    inner_cloud_(new pcl::PointCloud<pcl::PointXYZI>),
    t_(t),
    p_(p),
    q_(q),
    r_(r),
    max_distance_(max_distance)
{
}

void M2DP::preProcess(pcl::PointCloud<pcl::PointXYZI>::Ptr source)
{
    pcl::PCA<pcl::PointXYZI> pca;
    pca.setInputCloud(source);
    pcl::PointCloud<pcl::PointXYZI>::Ptr tmp_inner_cloud(new pcl::PointCloud<pcl::PointXYZI>);
    pca.project(*source, *tmp_inner_cloud);

    for (auto& point : tmp_inner_cloud->points) {
      double d = getLength(point);
      if (d <= max_distance_) inner_cloud_->push_back(point);
    }

    if (r_ < 1.e-6) {
      PCL_ERROR("r is too small.\n");
      return;
    }
    l_ = std::ceil(std::sqrt(max_distance_ / r_));
    A_.resize(p_ * q_, l_ * t_);

}


Eigen::VectorXi M2DP::singleViewProcess(double theta, double phi)
{
    Eigen::Vector3f m;
    m << std::cos(theta) * std::cos(phi), std::cos(theta) * std::sin(phi),
        std::sin(theta);

    // refer to the matlab code in git (according to the paper)
    Eigen::Vector3f projected_x_axis =
        Eigen::Vector3f(1., 0., 0.) -
        (Eigen::Vector3f(1., 0., 0.).transpose() * m).norm() * m;
    Eigen::Vector3f projected_y_axis = m.cross(projected_x_axis);

    const double m_2_pi = M_PI * 2.;
    auto calulate_polar = [&m_2_pi](const Eigen::Vector2f& p) {
      PolarCoordinate coord;
      coord.length = p.norm();
      coord.angle = std::atan2(p[1], p[0]);
      if (coord.angle < 0.) coord.angle += m_2_pi;

      return coord;
    };

    Eigen::VectorXi row;
    row.resize(l_ * t_, 1);
    for (int i = 0; i < l_ * t_; ++i) row[i] = 0;
    const double angle_step = m_2_pi / t_;
    for (auto& point : inner_cloud_->points) {
      Eigen::Vector3f p(point.x, point.y, point.z);
      Eigen::Vector2f projected_point((p.transpose() * projected_x_axis).norm(),
                                      (p.transpose() * projected_y_axis).norm());

      PolarCoordinate coord = calulate_polar(projected_point);
      int32_t l_index = std::floor(std::sqrt(coord.length / r_));
      // avoid over border
      if (l_index > l_ - 1) {
        l_index = l_ - 1;
      }
      int32_t t_index = std::floor(coord.angle / angle_step);
      if (t_index > t_ - 1) {
        t_index = t_ - 1;
      }
      int32_t index = l_index * t_ + t_index;
      //         std::cout << index << "  " << l_index << "  " << t_index << "  "
      //         << coord.angle << std::endl;
      row[index]++;
    }

    return row;
}


bool M2DP::setInputSourceCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr source)
{
    if (source->empty()) {
      PCL_ERROR("source is empty.\n");
      return false;
    }

    // step1 pre processing
    preProcess(source);

    // step2 calculate A
    const double theta_step = M_PI / p_;
    const double phi_step = M_PI_2 / q_;
    for (int p = 0; p < p_; ++p)
      for (int q = 0; q < q_; ++q) {
        Eigen::VectorXi row = singleViewProcess(p * theta_step, q * phi_step);
        A_.block(p * q_ + q, 0, 1, l_ * t_) = row.transpose().cast<float>();
      }

    // SVD and get the final Descriptor
    Eigen::JacobiSVD<Eigen::MatrixXf> svd(
        A_, Eigen::ComputeThinU | Eigen::ComputeThinV);
    Eigen::MatrixXf V = svd.matrixV(), U = svd.matrixU();
    Eigen::VectorXf v1 = V.col(0), u1 = U.col(0);  // paper algorithm1.15

    descriptor1_.resize(u1.rows() + v1.rows(), 1);
    descriptor1_ << u1, v1;
    return true;
}


bool M2DP::setInputTargetCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr source)
{
    if (source->empty()) {
      PCL_ERROR("source is empty.\n");
      return false;
    }

    // step1 pre processing
    preProcess(source);

    // step2 calculate A
    const double theta_step = M_PI / p_;
    const double phi_step = M_PI_2 / q_;
    for (int p = 0; p < p_; ++p)
      for (int q = 0; q < q_; ++q) {
        Eigen::VectorXi row = singleViewProcess(p * theta_step, q * phi_step);
        A_.block(p * q_ + q, 0, 1, l_ * t_) = row.transpose().cast<float>();
      }

    // SVD and get the final Descriptor
    Eigen::JacobiSVD<Eigen::MatrixXf> svd(
        A_, Eigen::ComputeThinU | Eigen::ComputeThinV);
    Eigen::MatrixXf V = svd.matrixV(), U = svd.matrixU();
    Eigen::VectorXf v1 = V.col(0), u1 = U.col(0);  // paper algorithm1.15

    descriptor2_.resize(u1.rows() + v1.rows(), 1);
    descriptor2_ << u1, v1;
    return true;
}

double M2DP::matchTwoM2dpDescriptors()
{
  if (descriptor1_.rows() != descriptor2_.rows() || descriptor1_.rows() < 10) {
    PCL_ERROR("The Descriptors do not match.\n");
    return -1.;
  }

  // refer to paper
  // "Spin-Images: A Representation for 3-D Surface Matching"
  // section 2.4 page.28
  auto N = descriptor1_.rows();
  double score = (N * descriptor1_.dot(descriptor2_) - descriptor1_.sum() * descriptor2_.sum()) /
                 std::sqrt((N * descriptor1_.dot(descriptor1_) - std::pow(descriptor1_.sum(), 2)) *
                           (N * descriptor2_.dot(descriptor2_) - std::pow(descriptor2_.sum(), 2)));

  //   double lamda = 1.;
  //   score = std::pow( std::atanh(score), 2 ) - lamda / ( N - 3 );
  return std::fabs(score);
}



